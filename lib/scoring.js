...
// Téléphone + Email = enrichissement (Google Maps / scraping)

function scoreJoignabilite(entreprise) {
  let score = 0;

  if (entreprise.dirigeant?.nom) score += 30; // SIRENE
  if (entreprise.telephone)      score += 40; // enrichissement Google Maps
  if (entreprise.email)          score += 30; // enrichissement scraping
